...
class PlayerAttacker(BasePlayer):
    ...
    def execute_action(self,
                        ...
                        ball: Ball = None) -> dict:
            """
            Execute one step with action vector (Box(7,)):
                [dx, dy, pass_flag, shoot_flag, power, dir_x, dir_y]

            Args:
                action (np.ndarray): Agent action vector.
                time_step (float): Duration of simulation step (1/FPS).
                x_range (float): Field width in meters.
                y_range (float): Field height in meters.
                ball (Ball): Ball instance for interaction.

            Returns:
                dict: Context info for reward calculation.
            """

            # UNPACK ACTION
            dx, dy, pass_flag, shoot_flag, power, dir_x, dir_y = action
            move_vec = np.array([dx, dy], dtype=np.float32)
            norm = np.linalg.norm(move_vec)

            # Facing direction update
            if norm > 1e-6:
                move_dir = move_vec / norm
                self.last_action_direction = move_dir
            else:
                move_dir = np.zeros(2, dtype=np.float32)

            # APPLY MOVEMENT (BASE CLASS)
            ...
            desired_direction = desired_direction / dir_norm if dir_norm > 1e-6 else np.array([1.0, 0.0])
            ...
            # If no ball → only movement
            if ball is None or ball.get_owner() != self.agent_id:
                self.current_action = "move"
                return context

            # Invalid combo (pass + shoot at same time)
            ...
            if shoot_flag > 0.5:
                ...
                # Alignment with goal (simplified target = center)
                goal_direction = np.array([1.0, 0.5]) - np.array(self.position)
                goal_norm = np.linalg.norm(goal_direction)
                goal_direction = goal_direction / goal_norm if goal_norm > 1e-6 else np.array([1.0, 0.0])
