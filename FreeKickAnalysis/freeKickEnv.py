...
class FreeKickEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.current_step == 1:
                ...
                if player_id == 0:  # A0, the free kick taker
                    if self.ball_owner == 0:
                        if action == 0:  # Pass to A1
                            ...
                            shot_power = random.uniform(0.5, 1.0)  # Shot power
                            shot_angle = random.uniform(-0.1, 0.1)  # Shot angle
                            self.ball[0] += shot_power * 0.1
                        ...
                        rewards[player_id] = -0.1  # Penalization if A0 doesn't have the ball
                        done = True
            ...
            for player_id in range(6):  # 6 players in total
                ...
                if self.ball_owner == player_id:
                    if action == 0:  # Pass to A1
                        ...
                        shot_power = random.uniform(0.5, 1.0)
            ...
            # --- Actions for defenders (D0, D1, D2), they can only move (dribble or change position) ---
        ...
        if self._check_goal():
            ...
            for player_id in range(3):  # A0, A1, A2
                rewards[player_id] += 1.0  # Reward for attackers when a goal is scored
        else:
            # Penalize defenders if a goal is conceded
            for player_id in range(3, 6):  # D0, D1, D2
                rewards[player_id] -= 0.5  # Penalty for defenders if goal is conceded
